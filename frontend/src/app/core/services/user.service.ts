...
export class UserService {
  ...
  changePassword(current_password: string, new_password: string): Observable<void> {
    return this.http.patch<void>(`${environment.apiUrl}/auth/change-password`, { current_password, new_password });
  }

  resetPassword(id: string, new_password?: string): Observable<{ new_password: string }> {
    return this.http.post<{ new_password: string }>(`${environment.apiUrl}/clients/${id}/reset-password`, new_password ? { new_password } : {});
  }
}
