...
from app.api.dependencies import get_db, get_current_actor, admin_required, client_resource_permission
...
def get_tasks(db: Session = Depends(get_db), admin_user = Depends(admin_required)):
    tasks = db.query(Task).all()
    return tasks

@router.get("/stage/{stage_id}", response_model=List[TaskRead])
def get_tasks_by_stage(stage_id: str, db: Session = Depends(get_db), actor = Depends(get_current_actor)):
...
def get_tasks_by_project(project_id: str, db: Session = Depends(get_db), actor = Depends(get_current_actor)):
...
def get_task(task_id: str, db: Session = Depends(get_db), actor = Depends(get_current_actor)):
...
def create_task(task_data: TaskCreate, db: Session = Depends(get_db), admin_user = Depends(admin_required)):
...
def update_task(task_id: str, task_data: TaskUpdate, db: Session = Depends(get_db), admin_user = Depends(admin_required)):
    task = db.get(Task, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
...
def delete_task(task_id: str, db: Session = Depends(get_db), admin_user = Depends(admin_required)):
    task = db.get(Task, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
